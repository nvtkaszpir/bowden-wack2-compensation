...
# Limit maxmimum extrusion adjustment to a factor of the actual extrusion (excluding travel moves)
...
def extruder_position_adjustment(x_position):
    return coefficient_a + coefficient_b * x_position + coefficient_c * x_position * x_position
...
class Gcode:
    ...
    def state(self) -> State:
        ...
        if self.command == "G1" or self.command == "G2" or self.command == "G3" or (self.command == "G0" and use_for_travel_moves is True):
...
def read_gcode_file(path: str) -> List[Gcode]:
    ...
    with open(path, "r", encoding='utf8') as readfile:
        ...
        for line in lines:
            ...
            elif gcode.command == "M221" and force_full_flow is True: # WACK: force full flow
            ...
            if gcode.command == "G1" or gcode.command == "G2" or gcode.command == "G3":
                ...
                if gcode.previous_state is not None and x_value is not None:
                    ...
                    e_xposition_adjust = extruder_position_adjustment(x_value) - extruder_position_adjustment(gcode.previous_state.X)
                    
                    x_change = x_value - gcode.previous_state.X
                    
                    if (travel_move is False or freeze_settle_on_travel is False): # if NOT travel move OR freeze settle on travel is DISABLED
                        if x_change > 0: 
                            ...
                            settling_current = min(settling_current + settling_positive * x_change, max_settling_positive)
                        elif x_change < 0:
                            # X- - leftwards movement
                            # settling towards 0! therefore, take the largest of the two
                            settling_current = max(settling_current + settling_negative  * x_change, 0)
                        
                        if (settling_current < 0 or settling_current > max_settling_positive):
                            print("Warning: settling_current out of bounds: " + str(settling_current) + "check the settling_positive and settling_negative values (both must be a positive number)")
                        
					# convert to relative extrusion change
                    e_adjust_relative = (e_xposition_adjust + settling_current)/180 * x_change
                    ...
                    if e_value is not None:
                        ...
                        e_adjust_relative = min(e_adjust_relative, e_value * adjustment_hardlimit_factor)
                        e_adjust_relative = max(e_adjust_relative, -e_value * adjustment_hardlimit_factor)
                    ...
                    if verbose:
                            gcodes.append(";ABS E adj:" + "{:.5f}".format(e_xposition_adjust) + " settl:" + "{:.5f}".format(settling_current) + " Δx:" + "{:.3f}".format(x_change)) 
                            gcodes.append(";REL E adj:" + "{:.5f}".format(e_adjust_relative)) 
...
def main():
    parser = argparse.ArgumentParser(description='Bowden wack2 compensation post-process')
